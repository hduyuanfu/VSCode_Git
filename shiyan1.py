...
def train():
    ...
    device = torch.device("cuda:1,2" if torch.cuda.is_available() else "cpu")  # 有cuda这个架构就使用0卡，无则cpu;device(可以指定任何设备，cpu,哪块或哪几块显卡)
    ...
    if device.type == 'cuda':
        gnet = nn.DataParallel(gnet, [1, 2])
        dnet = nn.DataParallel(dnet, [1, 2])
    ...
    for i in range(len(img_list)):
        writer.add_image('fake', img_list[i], i)
    '''
    writer.add_graph(dnet, input_to_model=(torch.rand(opt.batch_size, 3, 96, 96), ), verbose=True)
    writer.add_graph(gnet, input_to_model=(torch.rand(opt.batch_size, op.nd, 1, 1), ), verbose=True)
    '''
    writer.close()
...
def generate():
    ...
    dnet = torch.load('dnet.pth').to(device)#可能需要从其他GPU移动到0号，若满足条件则不作为
    ...
    writer = SummaryWriter(comment='generate')
    writer.add_image('fake', fake_img)
    writer.close()
